...
def extract_images(filename):
    ...
    with gzip.open(filename) as bytestream:
        ...
        if magic != 2051:
            raise ValueError("Invalid magic number %d in KMNIST image file: %s" % (magic, filename))
...
def extract_labels(filename, one_hot=False):
    ...
    with gzip.open(filename) as bytestream:
        magic = _read32(bytestream)
        if magic != 2049:
            raise ValueError("Invalid magic number %d in KMNIST label file: %s" % (magic, filename))
